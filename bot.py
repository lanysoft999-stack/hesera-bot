...
def admin_add_prod_start(call):
    cat_id = int(call.data.split("_")[3])
    user_states[call.from_user.id] = f'add_prod_{cat_id}'
    bot.send_message(call.message.chat.id, "📦 Шаг 1. Введите через пробел: `Название Цена`\nПример: `stoper 1.5 sek 67`")

@bot.message_handler(func=lambda message: message.chat.id == ADMIN_ID and user_states.get(message.chat.id, "").startswith("add_prod_"))
def admin_add_prod_desc(message):
    cat_id = int(user_states[message.chat.id].split("_")[2])
    try:
        parts = message.text.rsplit(" ", 1)
        name, price = parts[0], int(parts[1])
        user_states[message.chat.id] = f'add_prod_desc_{cat_id}'
        user_states[f'{message.chat.id}_prod_name'] = name
        user_states[f'{message.chat.id}_prod_price'] = price
        bot.send_message(message.chat.id, "📝 Шаг 2. Отправьте **описание** товара:")
    except:
        bot.send_message(message.chat.id, "❌ Ошибка. Пишите: `Название Цена`")

@bot.message_handler(func=lambda message: message.chat.id == ADMIN_ID and user_states.get(message.chat.id, "").startswith("add_prod_desc_"))
def admin_add_prod_photo(message):
    cat_id = int(user_states[message.chat.id].split("_")[3])
    user_states[f'{message.chat.id}_prod_desc'] = message.text
    user_states[message.chat.id] = f'add_prod_photo_{cat_id}'
    bot.send_message(message.chat.id, "🖼 Шаг 3. Отправьте **фото** товара. (Напишите `/skip_photo` если фото нет)")

@bot.message_handler(content_types=['photo'], func=lambda message: message.chat.id == ADMIN_ID and user_states.get(message.chat.id, "").startswith("add_prod_photo_"))
def admin_add_prod_file(message):
    cat_id = int(user_states[message.chat.id].split("_")[3])
    user_states[f'{message.chat.id}_prod_photo'] = message.photo[-1].file_id
    user_states[message.chat.id] = f'add_prod_file_{cat_id}'
    bot.send_message(message.chat.id, "📁 Шаг 4. Отправьте **файл** (.txt, .apk, .zip). (Напишите `/skip_file` если файла нет)")

@bot.message_handler(func=lambda message: message.chat.id == ADMIN_ID and user_states.get(message.chat.id, "").startswith("add_prod_photo_") and message.text == "/skip_photo")
...
@bot.message_handler(content_types=['document'], func=lambda message: message.chat.id == ADMIN_ID and user_states.get(message.chat.id, "").startswith("add_prod_file_"))
def admin_add_prod_finish(message):
    ...
    db_add_product(cat_id, 0, name, price, desc, photo, message.document.file_id)
    bot.send_message(message.chat.id, f"✅ Товар **{name}** за {price}₽ создан!")
    user_states.pop(message.chat.id, None)

@bot.message_handler(func=lambda message: message.chat.id == ADMIN_ID and user_states.get(message.chat.id, "").startswith("add_prod_file_") and message.text == "/skip_file")
